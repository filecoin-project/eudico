...
type HierarchicalCns interface {
	AddSubnet(ctx context.Context, wallet address.Address, parent address.SubnetID, name string, cns uint64, minerStake abi.TokenAmount, checkPeriod abi.ChainEpoch, p *hierarchical.ConsensusParams) (address.Address, error) // perm:write
	...
	SubnetGetValidators(ctx context.Context, id address.SubnetID) (string, error)                                                                                                                                              // perm:read
}
