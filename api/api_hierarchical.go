...
type HierarchicalCns interface {
	AddSubnet(ctx context.Context, wallet address.Address, parent address.SubnetID, name string, cns uint64, minerStake abi.TokenAmount, checkperiod abi.ChainEpoch, dm address.Address, n uint64) (address.Address, error) // perm:write
	...
	SubnetGetActorStateValidators(ctx context.Context, id address.SubnetID) (string, error)                                                                                                                                 // perm:read
}
