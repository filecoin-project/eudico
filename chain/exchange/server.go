...
func collectChainSegment(cs *store.ChainStore, req *validatedRequest) ([]*BSTipSet, error) {
	...
	for {
		...
		if req.options.IncludeMessages {
			bmsgs, bmincl, smsgs, smincl, crossmsg, crossincl, err := gatherMessages(cs, ts)
			if err != nil {
...
func gatherMessages(cs *store.ChainStore, ts *types.TipSet) ([]*types.Message, [][]uint64,
	...
	for _, block := range ts.Blocks() {
		bc, sc, crossc, err := cs.ReadMsgMetaCids(block.Messages)
		if err != nil {
