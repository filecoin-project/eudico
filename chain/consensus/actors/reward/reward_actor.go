...
func (a Actor) ExternalFunding(rt runtime.Runtime, params *FundingParams) *abi.EmptyValue {
	...
	if value.GreaterThan(rt.CurrentBalance()) {
		rt.Log(rtt.WARN, "reward actor balance %d below totalReward expected %d, paying out rest of balance", rt.CurrentBalance(), value)
		value = rt.CurrentBalance()
	}

	code := rt.Send(addr, builtin.MethodSend, nil, value, &builtin.Discard{})
	if !code.IsSuccess() {
		rt.Log(rtt.ERROR, "failed to send external funds to address with value: %v, code: %v", value, code)
	}
