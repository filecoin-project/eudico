// Package roundrobin implements round-robin consensus for testing purposes only.
// Roundrobin consensus is a permissioned consensus model where nodes are selected in predefined order to create blocks.
package roundrobin
...
const (
	MaxHeightDrift = 5
	ValidatorsEnv  = "EUDICO_RR_VALIDATORS"
)

var (
	log                     = logging.Logger("roundrobin-consensus")
	_   consensus.Consensus = &RoundRobin{}
)
...
func NewConsensus(
	...
	log.Infof("New round-robin consensus for %s subnet", subnetID)
...
func (bft *RoundRobin) ValidateBlock(ctx context.Context, b *types.FullBlock) (err error) {
	log.Infof("starting block validation process at @%d", b.Header.Height)

	if err := common.BlockSanityChecks(hierarchical.RoundRobin, b.Header); err != nil {
...
func (bft *RoundRobin) ValidateBlockPubsub(ctx context.Context, self bool, msg *pubsub.Message) (pubsub.ValidationResult, string) {
	if self {
...
func (bft *RoundRobin) minerIsValid(maddr address.Address) error {
...
func (bft *RoundRobin) IsEpochBeyondCurrMax(epoch abi.ChainEpoch) bool {
	if bft.genesis == nil {
		return false
	}
...
func (bft *RoundRobin) Type() hierarchical.ConsensusType {
	return hierarchical.RoundRobin
}
