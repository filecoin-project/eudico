...
func initGenesis(params *hierarchical.ConstructParams) ([]byte, error) {
	...
	// parent := params.Parent.Parent + "/" + params.Parent.Actor.String()
	parent := "/root"
	testAddr, _ := address.NewIDAddress(1000)
	netName := address.NewSubnetID(address.SubnetID(parent), testAddr)
	err = WriteGenesis(netName, 1 /* params.Consensus*/, address.Undef, vreg, rem,
		params.CheckPeriod, 1000, buf)
	if err != nil {
		return nil, err
	}
...
func WriteGenesis(
	...
	case hierarchical.Delegated:
		...
		if err != nil {
			return xerrors.Errorf("failed make genesis block for %s: %w", hierarchical.ConsensusName(cns), err)
		}
	case hierarchical.PoW, hierarchical.Tendermint, hierarchical.Mir, hierarchical.Dummy:
