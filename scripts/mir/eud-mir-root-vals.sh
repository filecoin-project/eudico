...
export NODE_4_PATH="$HOME/.eudico-node4"
...
export NODE_4_NETADDR="$NODE_4_PATH/.netaddr"
...
NODE_4_DAEMON_LOG="./eudico_daemon_4.log"
NODE_4_MINER_LOG="./eudico_miner_4.log"
...
tmux new-session -d -s "mir" \; \
  ...
  \
  split-window -t "mir:2" -v \; \
  split-window -t "mir:2.0" -h \; \
  split-window -t "mir:2.2" -h \; \
  \
  send-keys -t "mir:0.0" "
