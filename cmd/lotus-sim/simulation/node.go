...
func (nd *Node) LoadSim(ctx context.Context, name string) (*Simulation, error) {
	...
	sim.StateManager, err = stmgr.NewStateManager(nd.Chainstore, filcns.NewTipSetExecutor(), nil, vm.Syscalls(mock.Verifier), us, nil)
	if err != nil {
...
func (nd *Node) CreateSim(ctx context.Context, name string, head *types.TipSet) (*Simulation, error) {
	...
	sm, err := stmgr.NewStateManager(nd.Chainstore, filcns.NewTipSetExecutor(), nil, vm.Syscalls(mock.Verifier), filcns.DefaultUpgradeSchedule(), nil)
	if err != nil {
